...
class Comments extends Actioner {
    ...
    $lang = PM::plugLocales(CommentsToMoguta::$pluginName);
		...
		if (!$infoUser&&(empty($_POST['name'])||empty($_POST['email'])||empty($_POST['comment']))) {
			$this->messageError = $lang['ERROR_INPUT'];
			return false;
		}

		// если пользователь авторизован но не заполнил поле с комментаринем
		if (empty($_POST['comment'])) {
			$this->messageError = $lang['ERROR_COMMENT'].$_POST['comment'];
			return false;
		}
	
		$name = !empty($infoUser->name) ? $infoUser->name : '';
		$email = !empty($infoUser->email) ? $infoUser->email: '';
		...
		if (!preg_match('/^[-._a-zA-Z0-9]+@(?:[a-zA-Z0-9][-a-zA-Z0-9]{0,61}+\.)+[a-zA-Z]{2,10}$/', $email)) {      
				$this->messageError = $lang['ERROR_VALID_EMAIL'];
				return false;
			}
		}
		...
		DB::query($sql);
		$this->messageSucces = '
			<span class="c-msg-green">'.$lang['SUCCESS_SEND'].'</span>
				<br >
			<span class="c-msg-red">'.$lang['SUCCESS_MODERATE'].'</span>';
		return true;
	}
	...
	public function uploadImg($files) {
		...
		foreach ($files['comments_file_input']['name'] as $key => $value) {
			...
			$imagePath = $files['comments_file_input']['tmp_name'][$key];
			switch(strtolower($files['comments_file_input']['type'][$key])){
				case 'image/jpeg':
					$image = imagecreatefromjpeg($imagePath);
					if (method_exists('Upload', 'rotateImageByExif')) {
						$image = Upload::rotateImageByExif($image, $imagePath);
					}
					$type = 'jpeg';
    ...
    //удаление доступно только модераторам и админам.
		...
		$this->messageSucces = $lang['SUCCESS_DELETE'];
    ...
    USER::AccessOnly('1,4','exit()');
		$this->messageSucces = $lang['SUCCESS_EDIT'];
		$res = DB::query(
